...
def find_customers(customers: pd.DataFrame, orders: pd.DataFrame) -> pd.DataFrame:
    ...
    # Redo using isin(). We are using '~' to flip the 'isin()' to find all the values that are not in the orders 'customerId'. next we say we only want to keep the column 'name'. and finally change the column title from 'name' to 'Customers'
